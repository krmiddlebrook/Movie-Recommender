        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                if (line.contains("\"")) {
                    ...
                    movieMap.put(Integer.parseInt(movie[0]), movie[1]);
//                    System.out.println("movie [id= " + movie[0] + ", title= " + movie[1]);
                }
                else {
                    String[] movie = line.split(",");
                    movieMap.put(Integer.parseInt(movie[0]), movie[1]);
...
//            System.out.println("movieId = " + id + " title = " + movieMap.get(id));
//        }
...
//            usersData.insert();
            String line;
            BufferedReader br;
            br = new BufferedReader(new FileReader(ratingsFilename));
            br.readLine();
            //skip the first row of data. Continue reading in data until the end of the file
            while ((line = br.readLine()) != null) {
                String[] rating = line.split(",");
                usersData.insert(Integer.parseInt(rating[0]), Integer.parseInt(rating[1]), Double.parseDouble(rating[2]));
//                System.out.println("rating [userId= " + rating[0] + ", movieId=" + rating[1] + ", rating=" + rating[3] + "]");
            }
        } catch(IOException e2){
            System.out.println("File not found.");
        }
    ...
    public void findRecommendations(int userid, int num, String filename) {
        ...
        UserNode similarUser =  usersData.findMostSimilarUser(userid);
        UserNode user = usersData.get(userid);
        int[] similarUserFavMovies = similarUser.getFavoriteMovies(num);

        try {
            FileWriter file = new FileWriter(filename);
            for (int i = 0; i < similarUserFavMovies.length; i++) {
                if (!user.hasSeenMovie(similarUserFavMovies[i])) {
                    file.write(movieMap.get(similarUserFavMovies[i]) + '\n');
                }
            }
            file.close();
        }
    ...
    public void findAntiRecommendations(int userid, int num, String filename) {
        ...
        UserNode similarUser = usersData.findMostSimilarUser(userid);
        UserNode user = usersData.get(userid);
        int[] similarUserWorstMovies = similarUser.getLeastFavoriteMovies(num);

        try {
            FileWriter file = new FileWriter(filename);
            for (int i = 0; i < similarUserWorstMovies.length; i++) {
                if (!user.hasSeenMovie(similarUserWorstMovies[i])) {
                    file.write(movieMap.get(similarUserWorstMovies[i]) + '\n');
                }
            }
            file.close();
        }
